...
export function parseGitStatus(output: string): Map<string, boolean> {
  ...
  while (eolIndex >= 0) {
    ...
    const deleted: boolean = workingTreeStatus === "D" || (workingTreeStatus === " " && output.charAt(startOfLine) === "D");
 ...
 * @beta
 */
export function getRepoRoot(cwd: string, gitPath?: string): string {
  let cachedResult: string | undefined = repoRootCache.get(cwd);
  if (!cachedResult) {
    const result = execa.sync(gitPath || "git", ["--no-optional-locks", "rev-parse", "--show-toplevel"], {
      cwd,
    });
 ...
 * @internal
 */
export function applyWorkingTreeState(rootDirectory: string, state: Map<string, string>, gitPath?: string): void {
  const statusResult = execa.sync(gitPath || "git", ["--no-optional-locks", "status", "-z", "-u", "--no-renames", "--"], {
    cwd: rootDirectory,
  });
  ...
  if (filesToHash.length) {
    ...
    // command length
    const hashObjectResult = execa.sync(gitPath || "git", ["hash-object", "--stdin-paths"], {
      cwd: rootDirectory,
 ...
 * @beta
 */
export function getRepoState(cwd: string, gitPath?: string): Map<string, string> {
  const rootDirectory: string = getRepoRoot(cwd, gitPath);

  const lsTreeResult = execa.sync(gitPath || "git", ["--no-optional-locks", "ls-tree", "-r", "-z", "--full-name", "HEAD", "--"], {
    cwd: rootDirectory,
  });
...
export function getRepoChanges(cwd: string, revision = "HEAD", gitPath?: string): Map<string, IFileDiffStatus> {
  ...
  const result = execa.sync(
    gitPath || "git",
    ["--no-optional-locks", "diff-index", "--color=never", "--no-renames", "--no-commit-id", "--cached", "-z", revision, "--"],
    {
      cwd: rootDirectory,
    }
  );

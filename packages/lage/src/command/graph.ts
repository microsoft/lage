...
export async function graph(cwd: string, config: Config) {
  ...
  const edges = createPackageGraph(allPackages, {namePatterns: pattern, includeDependencies: config.includeDependencies, includeDependents: config.includeDependents, withDevDependencies: config.withDevDependencies, withPeerDependencies: config.withPeerDependencies});

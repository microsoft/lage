...
export async function runTasks(options: { workspace: Workspace; context: RunContext; config: Config }) {
  const { workspace, context, config } = options;

  //const priorityMap = getPriorityMap(config.priorities);
  const pipeline = new Pipeline(workspace, config, context);

  for (const [id, targetDefinition] of Object.entries(config.pipeline)) {
    pipeline.addTargetDefinition(id, targetDefinition);
  }
